BEGIN
DELETE from office_credit_score;
 SET @i = 1 ;
SELECT @max_row :=max(sid) from organization_info;
 WHILE @i <= @max_row
DO  
SELECT @dd := org_code from organization_info where sid = @i;	#Routine body goes here...
INSERT INTO `office_credit_score` (`icard`,`level1_id`,`level1_name`,`score`,`created`,`updated`,`status`)  
...
(
	...
	(
		...
		(
			...
			FROM office_credit_data_aggregation
			WHERE icard=@dd
		) icard_t1,office_credit_score_rule
	...
	(
		...
		FROM
		(
			SELECT office_credit_data_aggregation.* FROM 
			(
				SELECT DISTINCT icard
				FROM office_credit_data_aggregation
				WHERE icard=@dd
			) icard_t2 
			LEFT JOIN office_credit_data_aggregation ON icard_t2.icard=office_credit_data_aggregation.icard	
		) a
		LEFT JOIN
		office_credit_score_rule b
		ON a.level4=b.level4_id AND a.level5=b.level5_name GROUP BY b.level4_id
	) d
	ON c.level4_id=d.level4_id AND c.icard=d.icard
) e
LEFT JOIN
office_credit_weight_flat f
